...
def BPFt1(x, N=200, Fc=150, Fs=44100, Wpass=1, Wstop=1, dens=20):
    ...
    Fstop = 2*Fc/4
    trans_width = 50
    #bands = np.asarray([0, Fstop - trans_width, Fstop, Fpass, Fpass + trans_width, Fs/2])/(Fs/2)
    bands = np.asarray([0, Fstop - trans_width, Fstop, Fpass, Fpass + trans_width, Fs / 2])
    gain = np.asarray([0, 1, 0])
    # Array of weights
    W = np.asarray([Wstop, Wpass])
    # Design filter
    b = remez(N+1, bands=bands, desired=gain, weight=None, type='bandpass', grid_density=dens, fs=Fs)  # DEVUELVE NANS WHY????
    # Apply filter
    y = lfilter(b, [1.0], x)
    return y, b
